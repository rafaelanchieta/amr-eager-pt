...
FILE = PATH+'/output_palavras.txt'

def call_palavras(sentence):
    """
    Function to run the PALAVRAS system
    """
    print(sentence)
    #cmd = 'sh '+PATH+'/run_palavras.sh %s' % sentence
    os.system('sh '+PATH+'/run_palavras.sh %s' % sentence)
    return _get_lemma(_palavras(FILE))

def _get_lemma(tokens):
    return tokens[0][1]['lemma']

def _palavras(file, expand=False):
    '''
